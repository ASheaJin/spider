        ...
        if current_count <= req_count:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    print('latest请求失败')
                    self.logger.error('latest请求失败，response.status_code != 200 url: %s' % url)
                    return None

            except requests.ConnectionError:
                print('再次连接访问！')
                self.logger.error('latest中requests.get()方法发生异常，再次连接访问！url: %s' % url)
                return self.get_latest_page(page,current_count + 1,req_count)
        else:
            print('latest请求超过限定次数')
            self.logger.error('latest请求失败，请求次数超过设定值:%d' % (req_count))
            return None
        ...
        if current_count <= req_count:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    print('byte请求失败')
                    self.logger.error('byte请求失败，response.status_code != 200 url: %s' % url)
                    return None
            except requests.ConnectionError:
                print('再次连接访问！')
